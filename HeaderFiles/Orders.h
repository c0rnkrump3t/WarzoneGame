...
public:
	...
	negotiate* clone();
	//void method used in the stream insertion operator so we can print the subclass object when it is pointed to by an from order
	void write(std::ostream&);
	//override stringToLog
	string stringToLog();
};
...
public:
	conquer();
	conquer(Territory* target, Player* p);
	~conquer();
	void execute();
	bool validate();
	//override stringToLog
